...
-define(Setup, fun() -> 
		       ...
		       mentions_db_test:setup_server()
	       end).
-define(Clearnup, fun({Pid, SQLite3Pid}) ->
			  mentions_db:stop(Pid),
     ...
     {setup, ?Setup, ?Clearnup,
      [
       fun() ->
	       ...
	       MessageId1 = mentions_db_test:get_message_id(TomId, -1),
	       MessageId2 = mentions_db_test:get_message_id(TomId, -2),
	       MessageId3 = mentions_db_test:get_message_id(MikeId, -1),

	       ?assertEqual({ok, MessageId1}, 
			    mentions_db:save_message_id(Pid, MessageId1)),

	       ?assertEqual({ok, MessageId2}, 
			    mentions_db:save_message_id(Pid, MessageId2)),

	       ?assertEqual({ok, MessageId3}, 
			    mentions_db:save_message_id(Pid, MessageId3))
       end
      ]
     }
    }.
...
setup_server() ->
    ...
    register(db_server, mentions_db:start(mike, SQLitePid)),
